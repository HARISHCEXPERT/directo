...
export default function SignupPage() {
  ...
  if (success) {
    return (
      <div style={{ minHeight: '100vh', background: '#fafaf8', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,700&family=Syne:wght@800&display=swap');*{box-sizing:border-box;margin:0;padding:0;}`}</style>
        <div style={{ background: 'white', border: '1px solid #f0f0f0', borderRadius: 24, padding: 48, textAlign: 'center', maxWidth: 400, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.06)' }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>📧</div>
          <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, fontWeight: 800, color: '#1a1a1a', marginBottom: 8 }}>Check your email</h2>
          <p style={{ fontSize: 14, color: '#777', lineHeight: 1.7 }}>
            We sent a confirmation link to{' '}
            <span style={{ color: '#1a1a1a', fontWeight: 600 }}>{email}</span>.
            <br />Click it to activate your account.
          </p>
          <p style={{ fontSize: 12, color: '#bbb', marginTop: 20 }}>No spam. Ever.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fafaf8', display: 'flex', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      {/* LEFT PANEL */}
      <div style={{ display: 'none', width: '45%', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 60%, #f093fb 100%)', padding: '48px 40px', flexDirection: 'column', justifyContent: 'space-between' }} className="left-panel">
        ...
        <div>
          <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 34, fontWeight: 800, color: 'white', lineHeight: 1.2, marginBottom: 12 }}>
            Launch everywhere<br />before lunch.
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 15, lineHeight: 1.7 }}>
            AI fills 90+ directory forms automatically. You just review and submit.
          </p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            { n: '01', t: 'Add your product', d: 'Name, URL, description — 30 seconds' },
            { n: '02', t: 'AI fills every form', d: 'Extension auto-fills 90+ directories' },
            { n: '03', t: 'Track approvals', d: 'Backlinks, traffic, visibility — live' },
          ].map(s => (
            <div key={s.n} style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 14, padding: '14px 16px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              <span style={{ color: 'rgba(255,255,255,0.6)', fontWeight: 700, fontSize: 12, flexShrink: 0, marginTop: 2 }}>{s.n}</span>
              <div>
                <div style={{ color: 'white', fontWeight: 600, fontSize: 14, marginBottom: 2 }}>{s.t}</div>
                <div style={{ color: 'rgba(255,255,255,0.65)', fontSize: 13 }}>{s.d}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div style={{ width: '100%', maxWidth: 400 }}>
          ...
          <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 28, fontWeight: 800, color: '#1a1a1a', marginBottom: 6 }}>Create your account</h1>
          <p style={{ fontSize: 14, color: '#888', marginBottom: 28 }}>Start launching in under 2 minutes</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
            <div>
              <label style={{ fontSize: 13, color: '#555', fontWeight: 500, marginBottom: 6, display: 'block' }}>Full name</label>
            ...
            <div>
              <label style={{ fontSize: 13, color: '#555', fontWeight: 500, marginBottom: 6, display: 'block' }}>Password</label>
              <input className="input" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" onKeyDown={e => e.key === 'Enter' && handleSignup()} />
            </div>
          </div>
          ...
          <p style={{ fontSize: 12, color: '#bbb', textAlign: 'center', marginTop: 14 }}>🔒 Private & encrypted. No spam, ever.</p>
